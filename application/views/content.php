<div id = "haha">           

		<script src="<?php echo base_url('assets/test/js-image-slider.js')?>"></script>
	
		<link href="<?php echo base_url('assets/test/js-image-slider.css')?>" rel="stylesheet">

		<div id="sliderFrame">
			<div id="slider">				
				<img id="image1" src="<?php echo base_url('images/satu.jpg'); ?>"/>
				<img id="image1" src="<?php echo base_url('images/dua.jpg'); ?>"/>
				<img id="image1" src="<?php echo base_url('images/tiga.jpg'); ?>"/>
			</div>
			<div style="display: none;">
				<div id="cap1">
					Welcome to <a href="http://www.google.com/">SEMSAS</a>.
				</div>
				<div id="cap2">
					<em>HTML</em> caption. Link to <a href="http://www.google.com/">Google</a>.
				</div>
			</div>
		</div>


</div>
